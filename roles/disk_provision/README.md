...
root partition. For the example layout below:
...
<root partition>/etc/fstab                  # persistent, on the imaged disk
```

A rendered `20-02-root.conf` (the `grow` root partition) looks like:

```ini
# Ansible managed
# Generated by david_igou.armbian.disk_provision.
# Source: layout entry id=02-root
