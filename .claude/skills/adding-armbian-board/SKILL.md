...
- `docs/docsite/rst/boot-mode-override.rst`, `docs/docsite/rst/retry-configuration.rst` — boot-mode convergence + retry knobs (post-staging); the retry primitives live in `playbooks/tasks/cold_boot_with_retry.yml` and friends (no role-level wrapper any more).
