...
**Architecture:** Per-host inline `armbian_local_disks` list of disk bindings. Each binding's `layout` is translated to `systemd-repart` `.repart.d/*.conf` files on the target board. `disk_provision` refactored from single-partition into single-disk role (the playbook loops over multi-disk bindings). `pxelinux_render` template extended for `armbian_extra_modes`. `converge_boot_mode.yml` refactored so its body is includable from a `block/rescue` for the lifecycle wrapper's auto-revert.
  ...
  main:
    ...
    options:
      disk_binding:
        ...
        required: true
        description: >-
          One entry from armbian_local_disks. Shape:
...
mount_dir_base: "/var/lib/armbian/disk_provision_mnt"
render_only: false
...
- name: "Validate boot_mode is built-in or in extra_modes"
  ansible.builtin.assert:
    ...
    fail_msg: >-
      ...
      nfs|sd|local or a key in armbian_extra_modes. Defined
...
- name: "Assert custom mode rootfs source matches verify_match"
  ansible.builtin.assert:
    ...
    fail_msg: >-
      ...
      Define armbian_extra_modes[{{ boot_mode }}].verify_match
...
# armbian_router defined per host.

- name: "Plumbing check via reference playbook"
  ansible.builtin.include_tasks: "{{ armbian_plumbing_check_tasks | default('../routeros/tasks/plumbing_check_one.yml') }}"
  run_once: true
...
- name: "Upload pxelinux.cfg via reference playbook tasks"
  ansible.builtin.include_tasks: "{{ armbian_pxelinux_upload_tasks | default('../routeros/tasks/upload_pxelinux_one.yml') }}"
  delegate_to: "{{ armbian_router }}"
...
**Note for the implementing engineer:** the `armbian_plumbing_check_tasks` / `armbian_pxelinux_upload_tasks` references assume there are per-host-invocable tasks files in `playbooks/routeros/tasks/`. If only the per-router-batch reference playbooks exist today (`routeros/upload_pxelinux_cfg.yml` runs across all hosts at once), this task may need to either (a) add `_one.yml` variants of those reference playbooks, or (b) batch-run the upload outside the loop in `_lifecycle_set_and_verify.yml`. **Read `playbooks/routeros/upload_pxelinux_cfg.yml` and `playbooks/routeros/tasks/upload_pxelinux_one.yml` before implementing to confirm which variant exists.**
...
- name: "Lifecycle: set + verify {{ target_boot_mode }}"
  block:
    ...
    - name: "Lifecycle: capture diagnostic bundle before revert"
      ...
      vars:
        bundle_dir: "{{ armbian_diagnostic_bundle_dir | default('./diagnostics') }}/{{ inventory_hostname }}-{{ ansible_date_time.iso8601_basic_short }}"
    ...
    - name: "Lifecycle: fail loudly with diagnostic bundle pointer"
      ansible.builtin.fail:
        msg: >-
          ...
          {{ armbian_diagnostic_bundle_dir | default('./diagnostics') }}/{{ inventory_hostname }}-{{ ansible_date_time.iso8601_basic_short }}/
```
...
#
# Requires:
#   armbian_local_disks: list of disk_binding dicts (per host)
#   armbian_boot_mode: local (per host or via -e)
#
# Phases:
#   1. Set boot mode to nfs and verify board is NFS-rooted.
#   2. Cross-binding validation (mount-path collisions, exactly one /).
#   3. Loop disk_provision over armbian_local_disks.
    ...
    - name: "Cross-binding validate: no two disks share a mount path"
      ansible.builtin.assert:
        ...
        fail_msg: >-
          ...
          Mount paths: {{ armbian_local_disks | map(attribute='layout') | flatten | selectattr('mount', 'defined') | map(attribute='mount') | list }}
...
Expected: all phases A–F succeed. Inventory for orange-pi-5-max-01 must have `armbian_local_disks` defined with the full ESP+boot+var+root layout per the spec, plus `armbian_poe_switch` / `armbian_poe_port`. **If the inventory entry doesn't have these yet, add them first** — that's a prerequisite for the test, not part of this task's commit.
...
```markdown
| 10 | `reprovision_to_local.yml` | Once per board (or whenever layout changes) | Headless full-lifecycle: boot board into NFS → loop disk_provision over `armbian_local_disks` → flip pxelinux to local → verify. Auto-reverts to nfs on local-boot failure with a diagnostic bundle captured. |
```
...
`armbian_local_disks` (a list of disk bindings, each with a
...
- Per-host inline `armbian_local_disks` DSL for declarative multi-partition, multi-disk layouts.
...
- `pxelinux_render` extended with `armbian_extra_modes` for user-defined named boot modes.
